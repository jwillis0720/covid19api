...
def predict(sub_df, days):
    ...
    for metric in ["confirmed", "deaths"]:
        ...
        stepwise_model = auto_arima(
            fit, seasonal=False, trace=False, out_of_sample_size=2, error_action="ignore", suppress_warnings=True
        )
...
def backfill_new_counties(df, key=["county", "state", "FIPS"]):
    date_range = pd.date_range(pd.to_datetime("2020-1-22"), df["Date"].max(), freq="1D")
    unique_group = key
    ...
    assert (all_concat.groupby(key).count() == len(date_range)).all().all()
    return all_concat
...
JHU_DF_AGG_COUNTRY = (
    jhu_time.sort_values("confirmed")[::-1]
    .groupby(["Date", "country"])
    .agg({"lat": "first", "lon": "first", "confirmed": "sum", "deaths": "sum", "country_code": "first"})
    .reset_index()
)
...
merge_country.loc[merge_country["pop_est"].isna(), "pop_est"] = merge_country.loc[
    merge_country["pop_est"].isna(), :
].apply(lambda x: manual_lookup_pop[x["country"]], axis=1)
merge_country.loc[merge_country["pop_est"] < 0, "pop_est"] = merge_country.loc[merge_country["pop_est"] < 0, :].apply(
    lambda x: manual_lookup_pop[x["country"]], axis=1
)
merge_country["state"] = ""
merge_country["county"] = ""
merge_country = merge_country[
    ["Date", "country", "state", "county", "lat", "lon", "pop_est", "confirmed", "deaths", "country_code"]
].rename({"pop_est": "pop"}, axis=1)
...
county_info = pd.read_csv(
    "https://raw.githubusercontent.com/jackparmer/mapbox-counties/master/lat_lon_counties.csv",
    index_col=0,
    parse_dates=True,
    thousands=",",
)
...
ny_times_counties = (
    pd.read_csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-counties.csv", parse_dates=["date"])
    .rename({"date": "Date", "cases": "confirmed", "fips": "FIPS"}, axis=1)
    .fillna(0)
)
...
merged_counties = merged_counties[
    ["Date", "country", "state", "county", "Latitude", "Longitude", "Population(2010)", "confirmed", "deaths", "FIPS"]
].rename({"Latitude": "lat", "Longitude": "lon", "Population(2010)": "pop"}, axis=1)
...
updater = (
    merged_counties.loc[(merged_counties["lat"].isna()) & (merged_counties["county"] == "Unknown")][key]
    .drop_duplicates()
    .merge(state_info, left_on="state", right_on="State")[key + ["Latitude", "Longitude", "Population(2010)"]]
    .rename({"Longitude": "lon", "Latitude": "lat", "Population(2010)": "pop"}, axis=1)
)
manual_update = [
    {
...
ny_times_states = (
    pd.read_csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-states.csv", parse_dates=["date"])
    .rename({"date": "Date", "cases": "confirmed", "fips": "FIPS"}, axis=1)
    .fillna(0)
)
...
# Add confirmed text
MASTER_ALL["Text_Confirmed"] = (
    MASTER_ALL["forcast"].apply(lambda x: "" if not x else "**Predicted**<br>")
    + MASTER_ALL["location"]
    + "<br> Total Cases: "
    + MASTER_ALL["confirmed"].apply(lambda x: "{:,}".format(int(x)))
)
MASTER_ALL["Text_Deaths"] = (
    MASTER_ALL["forcast"].apply(lambda x: "" if not x else "**Predicted**<br>")
    + MASTER_ALL["location"]
    + "<br> Total Deaths: "
    + MASTER_ALL["deaths"].apply(lambda x: "{:,}".format(int(x)))
)

# Make MASTER PID
key = ["country", "state", "county", "granularity"]
MASTER_PID = (
    MASTER_ALL.loc[MASTER_ALL["forcast"] == False]
    .groupby(["country", "state", "county", "granularity"])
    .tail(1)
    .reset_index(drop=True)
    .reset_index()
    .rename({"index": "PID"}, axis=1)
)
MASTER_ALL = MASTER_ALL.merge(
    MASTER_PID[["PID", "country", "state", "county", "granularity"]], on=["country", "state", "county", "granularity"]
)
...
bins, ret_bins = pd.qcut(
    MASTER_ALL[(MASTER_ALL["confirmed"] >= 1) & (MASTER_ALL["country"] != "worldwide")]["confirmed"],
    q=[0, 0.5, 0.6, 0.70, 0.75, 0.8, 0.85, 0.9, 0.95, 0.999, 1],
    duplicates="drop",
    retbins=True,
)
yellows = ["#606056", "#6e6e56", "#7b7c55", "#898a54", "#979953", "#a5a850", "#b4b74d", "#c3c649", "#d2d643", "#e2e53c"]
reds = ["#5a4f4f", "#704d4d", "#854b4a", "#994746", "#ac4340", "#be3c3a", "#cf3531", "#e02a27", "#f01c19", "#ff0000"]
...
MASTER_ALL["CColor"] = (
    pd.cut(MASTER_ALL["confirmed"], bins=ret_bins, labels=yellows).astype(str).replace({"nan": "white"})
)
...
max_size = 1500
cut_me = MASTER_ALL[
    (MASTER_ALL["per_capita_confirmed"] >= 0)
    & (MASTER_ALL["country"] != "worldwide")
    & (MASTER_ALL["per_capita_confirmed"] != np.inf)
]
bins, ret_bins = pd.qcut(
    cut_me["per_capita_confirmed"],
    q=[0, 0.5, 0.6, 0.70, 0.75, 0.8, 0.85, 0.9, 0.95, 0.999, 1],
    duplicates="drop",
    retbins=True,
)
...
reds = ["#5a4f4f", "#704d4d", "#854b4a", "#994746", "#ac4340", "#be3c3a", "#cf3531", "#e02a27", "#f01c19", "#ff0000"]
labels = np.geomspace(1, max_size, num=len(ret_bins) - 1)
MASTER_ALL["per_capita_CSize"] = (
    pd.cut(MASTER_ALL["per_capita_confirmed"], bins=ret_bins, labels=labels).astype(float).fillna(0)
)
MASTER_ALL["per_capita_DSize"] = (
    pd.cut(MASTER_ALL["per_capita_deaths"], bins=ret_bins, labels=labels).astype(float).fillna(0)
)
MASTER_ALL["per_capita_CColor"] = (
    pd.cut(MASTER_ALL["per_capita_confirmed"], bins=ret_bins, labels=yellows[:])
    .astype(str)
    .replace({"nan": "white", "inf": "white"})
)
MASTER_ALL["per_capita_DColor"] = (
    pd.cut(MASTER_ALL["per_capita_deaths"], bins=ret_bins, labels=reds[:])
    .astype(str)
    .replace({"nan": "white", "inf": "white"})
)
...
for file in gs:
    upload_file(file, "jordanspubliccovidbucket", os.path.join("covid19data", os.path.basename(file)))
